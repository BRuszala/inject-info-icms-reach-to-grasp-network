...
art_data_path = ['C:\Users\bruszala\Documents\Projects\COT_ICMS\' monk '\' cortical_region '\StimTA\EMG_Data\'];
artChan = [242]; % For plexon files
art_filename = 'Q_20210805_ICMS_dim4_art_242_WLsort.nex';
...
if strcmpi(art_fullFile(end-3:end), '.plx')
    %Reading Plexon file (get markers and Xdsip and Ydisp Time series data)
    [~, ~, artTs_1, wave_1] = plx_waves_v(art_fullFile, artChan, 1);
    [~, ~, artTs_2, wave_2] = plx_waves_v(art_fullFile, artChan, 2);
    [~, ~, artTs_3, wave_3] = plx_waves_v(art_fullFile, artChan, 3);
    [~, ~, artTs_4, wave_4] = plx_waves_v(art_fullFile, artChan, 4);
        
    artTs = [artTs_1; artTs_2; artTs_3; artTs_4];
    artWaves = [wave_1; wave_2; wave_3; wave_4];
    
    [adfreq, n, ts, fn, EMG_data(1,:)] = plx_ad_v(art_fullFile, 'AD09');
    [~, ~, ~, ~, EMG_data(2,:)] = plx_ad_v(art_fullFile, 'AD10');
    [~, ~, ~, ~, EMG_data(3,:)] = plx_ad_v(art_fullFile, 'AD11');
    [~, ~, ~, ~, EMG_data(4,:)] = plx_ad_v(art_fullFile, 'AD12');
    [~, ~, ~, ~, EMG_data(5,:)] = plx_ad_v(art_fullFile, 'AD13');
    [~, ~, ~, ~, EMG_data(6,:)] = plx_ad_v(art_fullFile, 'AD14');

    [~, ~, ~, ~, EOM_data(1,:)] = plx_ad_v(art_fullFile, 'X_raw');
    [~, ~, ~, ~, EOM_data(2,:)] = plx_ad_v(art_fullFile, 'Y_raw');

else
    
    ns2_fullFile = [data_path ns2_filename];
    ns5_fullFile = ['D:\spikeProcessing\Input\' ns5_filename];
    %Reading Ripple file (get markers and Xdsip and Ydisp Time series data)
    nexInfo = readNexFile(art_fullFile);
    
    if useNs2
         EMG_fullFile = ns2_fullFile;   
    elseif useNs5
         EMG_fullFile = ns5_fullFile;   
    end
    
    % Extract the channel and unit index in the original recording system
    sortedNDX = cellfun(@(x) ~strcmpi(x.name(end), 'U'), nexInfo.neurons);
    
    nexChannels = cellfun(@(x) sscanf(x.name,'sig%d'), nexInfo.neurons);
    nexUnits = cellfun(@(x) x.unitNumber, nexInfo.neurons);
    nexTs = cellfun(@(x) {x.timestamps}, nexInfo.neurons);
    nexWf = cellfun(@(x) {x.waveforms}, nexInfo.waves);
        
    artTs = cat(1, nexTs{sortedNDX});
    artWaves = cat(2, nexWf{sortedNDX})';
...
%     EMG_data(8,:) = tempEMG.Data;
    ...
    tempEOM = FMLReadTimeseries(EMG_fullFile, 'analog 5');
    EOM_data(1,:) = tempEOM.Data;
    tempEOM = FMLReadTimeseries(EMG_fullFile, 'analog 6');
    EOM_data(2,:) = tempEOM.Data;
...
EOM_data_path = ['D:\COT_ICMS\' monk '\' cortical_region '\Ripple\'];
EOM_save_path = ['C:\Users\bruszala\Documents\Projects\COT_ICMS\' monk '\' cortical_region '\StimTA\'];
